...
all_configs = [
    ...
    for m in all_model_type
    for lr in all_lr
    for b in all_batch_size
    for o in all_optimizer
    for l in all_lambda_val
    for s in all_scheduler_step
    for w in all_weight_decay
]
...
def launch_job(config, save_dir):
    ...
    if config["model_type"] == "simple_finetune":
        ...
        model_suffix = (
            "-use_attr -weighted_loss multiple -data_dir %s -n_attributes %d -attr_loss_weight %.3f -normalize_loss"
            % (DATA_DIR, N_ATTR, config["lambda"])
        )
        if USE_RELU:
    ...
    command = (
        model_suffix
        + " -batch_size %d -lr %f -optimizer %s -weight_decay %f -scheduler_step %s"
        % (
            config["batch_size"],
            config["lr"],
            config["optimizer"],
