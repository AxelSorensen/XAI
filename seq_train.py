...
# Define your training loop here
def train_p_model(model_p,model_c, train_loader, criterion, optimizer, num_epochs=5):
...
model_c.load_state_dict(torch.load('seqc_model.pth'))
...
model_p.load_state_dict(torch.load('seqp_model.pth'))
optimizer = optim.Adam(model_p.parameters(), lr=0.001)
criterion = nn.CrossEntropyLoss()

train_p_model(model_p, model_c, train_loader, criterion, optimizer)
...
# Save the model
torch.save(model_c.state_dict(), 'seqc_model2.pth')
torch.save(model_p.state_dict(), 'seqp_model2.pth')
